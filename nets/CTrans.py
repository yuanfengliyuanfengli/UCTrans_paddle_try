...
import numpy as np
from paddle.nn import Dropout, Softmax, Conv2D, LayerNorm
# from torch.nn.modules.utils import _pair


logger = logging.getLogger(__name__)

class Channel_Embeddings(nn.Layer):
    """Construct the embeddings from patch, position embeddings.
    """
    def __init__(self,config, patchsize, img_size, in_channels):
        super().__init__()
        # print('img_size=',img_size,' ',type(img_size))
        # img_size = _pair(img_size)
        img_size=(img_size,img_size)
        # print('img_size after=',img_size,' ',type(img_size))
        ...
        self.patch_embeddings = Conv2D(in_channels=in_channels,
                                       ...
                                       stride=patch_size)
        # self.position_embeddings = nn.Parameter(paddle.zeros(1, n_patches, in_channels))
        self.position_embeddings = paddle.zeros(1, n_patches, in_channels)
...
class Attention_org(nn.Layer):
    ...
    def forward(self, emb1,emb2,emb3,emb4, emb_all):
        ...
        multi_head_Q1 = paddle.stack(multi_head_Q1_list, axis=1) if emb1 is not None else None
        multi_head_Q2 = paddle.stack(multi_head_Q2_list, axis=1) if emb2 is not None else None
        multi_head_Q3 = paddle.stack(multi_head_Q3_list, axis=1) if emb3 is not None else None
        multi_head_Q4 = paddle.stack(multi_head_Q4_list, axis=1) if emb4 is not None else None
        multi_head_K = paddle.stack(multi_head_K_list, axis=1)
        multi_head_V = paddle.stack(multi_head_V_list, axis=1)
        ...
        attention_scores1 = paddle.matmul(multi_head_Q1, multi_head_K) if emb1 is not None else None
        attention_scores2 = paddle.matmul(multi_head_Q2, multi_head_K) if emb2 is not None else None
        attention_scores3 = paddle.matmul(multi_head_Q3, multi_head_K) if emb3 is not None else None
        attention_scores4 = paddle.matmul(multi_head_Q4, multi_head_K) if emb4 is not None else None
        ...
        context_layer1 = paddle.matmul(attention_probs1, multi_head_V) if emb1 is not None else None
        context_layer2 = paddle.matmul(attention_probs2, multi_head_V) if emb2 is not None else None
        context_layer3 = paddle.matmul(attention_probs3, multi_head_V) if emb3 is not None else None
        context_layer4 = paddle.matmul(attention_probs4, multi_head_V) if emb4 is not None else None
...
class Block_ViT(nn.Layer):
    ...
    def forward(self, emb1,emb2,emb3,emb4):
        ...
        emb_all = paddle.concat(embcat,axis=2)
...
class ChannelTransformer(nn.Layer):
